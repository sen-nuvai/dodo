...
The API is tenant-scoped. Tenants own customers, invoices, payments, webhook registrations, and delivery rows. Customers own invoice references. Invoices store integer cents, USD currency, line items, an invoice state, and the associated payment ID. Payments store the tenant-scoped idempotency key, request fingerprint, provider result, attempt count, response status, and replayable JSON response data. Payment attempts preserve pending, succeeded, or failed provider outcomes independently of the payment summary. Webhook registrations hold an outbound URL and signing secret; webhook delivery rows are the PostgreSQL transactional outbox and retain exact payload bytes, event type, attempts, lease, retry time, and exhaustion state.

All durable records use UUID primary keys generated by the application or PostgreSQL UUID functions. The important lookup paths are indexed by tenant and idempotency key, invoice tenant/state, payment status, webhook event identity, and due delivery time. This keeps tenant isolation and claim/replay lookups predictable without introducing another datastore. At 100× scale, I would review query plans and partition or archive delivery history, add connection-pool/read-replica capacity where useful, and introduce operational metrics and retention policies before adding architectural components.
